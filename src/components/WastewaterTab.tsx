...
export function WastewaterInsertDialog({ open, onOpenChange }: Props) {
  ...
  const doVal = form.do_value ? Number(form.do_value) : null;
  const doWarning = doVal !== null && doVal < 2;
  ...
  const insertLog = useMutation({
    mutationFn: async () => {
      ...
      const payload: any = {
        ...
        do_value: form.do_value ? Number(form.do_value) : null,
    ...
    onSuccess: () => {
      ...
      setForm({
        ...
        sediment_volume: "", sedimentation_char: "", do_value: "", electricity_meter: "",
      ...
      <DialogContent className="rounded-3xl max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <Label className="text-xs font-semibold">DO (mg/L)</Label>
              <Input type="number" step="0.1" value={form.do_value} onChange={(e) => setForm({ ...form, do_value: e.target.value })} className="h-11 rounded-2xl" />
              {doWarning && (
                <p className="text-xs text-red-600 font-semibold mt-1">⚠ ค่า DO ต่ำกว่าเกณฑ์ (ควร ≥ 2 mg/L)</p>
              )}
            </div>
          </div>
          <div className="grid grid-cols-2 gap-3">
  ...
  const updateLog = useMutation({
    mutationFn: async () => {
      const payload: any = {
        ...
        do_value: editForm.do_value !== "" ? Number(editForm.do_value) : null,
            ...
            <table className="w-full text-sm min-w-[1100px]">
              <thead>
                <tr className="bg-emerald-50 border-b-2 border-emerald-200 text-emerald-800">
                  ...
                  <th className="px-2 py-2 text-center text-xs font-bold">DO (mg/L)</th>
        ...
        <DialogContent className="rounded-3xl max-w-lg max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-3">
            <div className="grid grid-cols-2 gap-3">
              ...
              <div><Label className="text-xs">DO (mg/L)</Label><Input type="number" step="0.1" value={editForm.do_value} onChange={(e) => setEditForm({ ...editForm, do_value: e.target.value })} className="h-10 rounded-xl" /></div>
