...
export default function Electricity() {
  ...
  // States สำหรับระบุช่วงวันที่เพื่อใช้กรองข้อมูล
  ...
  // ตรวจสอบว่าเป็นประเภทร้านค้าหรือไม่
  ...
  // 2. ✨ แก้ไข Bug สรุปค่าน้ำประปาประจำเดือน (ดึงสถิติตามเวลาจริงปี 2026 รองรับ พ.ศ./ค.ศ.)
  const currentMonthStats = React.useMemo(() => {
    ...
    logs.forEach((log: any) => {
      ...
      // ดักจับกรณีเครื่องผู้ใช้แปลงปีเป็น พ.ศ. เกิน 2500 ให้ทอนกลับเป็น ค.ศ.
      if (logYear > 2500) {
        logYear = logYear - 543;
      }

      // คัดกรองเอาเฉพาะรายการที่เกิดขึ้นในเดือนนี้
      if (logYear === currentYear && logDate.getMonth() === currentMonth) {
        ...
        // สรุปน้ำประปา (แปลงค่าอย่างปลอดภัยเพื่อไม่ให้ยอดคำนวณหลุดเป็น 0)
        if (log.current_water_value !== undefined && log.current_water_value !== null && log.current_water_value !== '-') {
          ...
          if (!isNaN(currentWater)) {
            // คำนวณหาผลต่างหน่วยที่ใช้ไปจริงในแต่ละรอบการจด
      ...
      html5QrCode.start(
        ...
        async (decodedText: string) => { 
          ...
          if (meterData) {
            ...
            setSelectedMeterId('');
            setMeterDisplayName('');
            toast({ variant: "destructive", title: "ไม่พบข้อมูล", description: `ลิงก์ QR นี้ยังไม่ได้ผูกในระบบ` });
          }
        }, 
        () => {}
      ).catch(() => {
  ...
  // ฟังก์ชัน Export Excel แยกหน้าแผ่นงาน
  const exportExcel = () => {
