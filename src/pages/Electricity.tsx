...
export default function Electricity() {
  ...
  // ดำเนินการบันทึกข้อมูลเข้าสู่ฐานข้อมูล (แก้ไขเช็กค่าอย่างปลอดภัย ไร้ส่วนเกิน)
  const handleSave = async () => {
    ...
    try {
      ...
      // ตรวจสอบความถูกต้องของค่างวดไฟบนหน้าจอโดยตรง ไม่ใช้ตัวแปรภายนอก
      ...
      // ส่งเฉพาะฟิลด์ที่มีอยู่ในตารางฐานข้อมูลจริง ๆ ของคุณพี่
                ...
                <div className="p-3 bg-blue-50/40 border border-blue-100 rounded-xl space-y-2 md:col-span-2">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    <div>
                      ...
                      <Input type="number" value={currentWaterValue} placeholder="ระบุเลขล่าสุด" onChange={(e) => setCurrentWaterValue(e.target.value)} className="text-center text-xs font-bold border-slate-300 focus:ring-2 focus:ring-blue-500 h-9 bg-white text-blue-700" />
                    </div>
                  </div>
                </div>
              )}
            ...
            <TableBody>
              {filteredLogs.length === 0 ? (
                ...
                filteredLogs.map((log: any) => {
                  const elecDiff = calculateElectricUnits(log.current_value || 0, log.previous_value || 0);
                  const prevWater = log.current_water_value ? getPreviousWaterValue(log, logs) : null;
                  
                  let waterDiff = 0;
                  if (log.current_water_value && prevWater !== null) {
                    if (log.current_water_value >= prevWater) {
                      waterDiff = log.current_water_value - prevWater;
                    } else if (prevWater > 9000 && log.current_water_value < 1000) {
                      waterDiff = (10000 - prevWater) + log.current_water_value;
                    }
                  }
                  ...
                  return (
                    <TableRow key={log.id} className="hover:bg-slate-50/50 transition-colors">
                      ...
                      <TableCell className="text-xs text-right font-bold text-indigo-600 bg-indigo-50/30 py-2.5">
                        {elecDiff.toLocaleString()}
                      </TableCell>
                      <TableCell className="text-xs text-right text-slate-500 py-2.5">{prevWater !== null ? prevWater : '-'}</TableCell>
                      ...
                      <TableCell className="text-xs text-right font-bold text-blue-600 bg-blue-50/30 py-2.5">
                        {log.current_water_value && prevWater !== null ? waterDiff.toLocaleString() : '-'}
                      </TableCell>
                    </TableRow>
                  );
                })
