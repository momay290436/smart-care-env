...
export default function Electricity() {
  ...
  const currentMonthStats = React.useMemo(() => {
    ...
    logs.forEach((log: any) => {
      ...
      if (logYear === currentYear && logDate.getMonth() === currentMonth) {
        // หากไม่มีค่า units_used บันทึกไว้ ให้ช่วยคำนวณส่วนต่างสดๆ ป้องกันแดชบอร์ดเป็น 0
        const eDiff = log.units_used !== undefined && log.units_used !== null ? log.units_used : (log.current_value - log.previous_value);
        if (eDiff > 0) totalElectricUnits += eDiff;
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // คำนวณส่วนต่างไฟฟ้าเพื่อจัดเก็บลงฟิลด์ units_used
      const electricUnitsUsed = currentVal - prevVal;
      ...
      if (isShop) {
        ...
        // หมายเหตุ: ไม่ใส่คอลัมน์ water_units_used เผื่อฐานข้อมูลของคุณหมอไม่ได้เจนโครงสร้างนี้ไว้ เพื่อหลีกเลี่ยง Db Error
      }
  ...
  const exportExcel = () => {
    const formatLogItem = (log: any) => ({
      ...
      'จำนวนหน่วยไฟที่ใช้ประจำงวด (หน่วย)': log.units_used !== undefined && log.units_used !== null ? log.units_used : (log.current_value - log.previous_value),
        ...
        <Card className="w-full shadow-sm border border-slate-200/80 bg-white rounded-2xl overflow-hidden">
          ...
          <CardContent className="space-y-3 pt-3 px-4 pb-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 bg-slate-50/50 p-3 sm:p-4 rounded-xl border border-slate-100">
              ...
              <div className="space-y-1 md:col-span-2 bg-indigo-50/40 p-3 border border-indigo-100
