...
export default function Electricity() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (isShop) {
        ...
        insertData.water_units_used = calculatedWaterUnits; // แนบหน่วยน้ำเผื่อโครงสร้างตารางรองรับ
      }
  ...
  const exportExcel = () => {
    const formatLogItem = (log: any) => ({
      ...
      'จำนวนหน่วยไฟที่ใช้ประจำงวด (หน่วย)': log.units_used !== undefined && log.units_used !== null 
        ? log.units_used 
        : calculateRolloverUnits(log.current_value, log.previous_value),
      'เลขมิเตอร์น้ำครั้งก่อน': log.previous_water_value || '-',
      'เลขมิเตอร์น้ำล่าสุด': log.current_water_value || '-',
      'จำนวนหน่วยน้ำที่ใช้ประจำงวด (หน่วย)': log.current_water_value !== null && log.previous_water_value !== null 
        ? calculateRolloverUnits(log.current_water_value, log.previous_water_value) 
        : '-'
    });
