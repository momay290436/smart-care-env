...
export default function WaterManagementPage() {
  // --- Tab Control เดิม ---
  const [activeTab, setActiveTab] = useState<'quality' | 'history' | 'emergencyHistory' | 'admin' | 'pm'>('history');
  const [historySubTab, setHistorySubTab] = useState<'meter' | 'chemical' | 'dailyWastewater' | 'statWastewater' | 'trash'>('statWastewater');

  // --- State สำหรับฟังก์ชัน PM อุปกรณ์ไฟฟ้า (ส่วนเพิ่มใหม่) ---
  const [pmCategory, setPmCategory] = useState<'mdb' | 'gen'>('mdb');
  const [selectedDevice, setSelectedDevice] = useState('ตู้ MDB อาคารหลัก');
  const [checkDate, setCheckDate] = useState(new Date().toISOString().split('T')[0]);
  const [notes, setNotes] = useState('');
  
  // สถานะการตรวจแต่ละรายการ (pass / fail)
  ...
  // ข้อมูลรายการตรวจเช็ค
  const pmConfig = {
    mdb: {
      title: 'บันทึกตรวจเช็ค: ตู้ควบคุมไฟฟ้า (MDB / Sub-Panel)',
      devices: ['ตู้ MDB อาคารหลัก', 'ตู้ Sub-Panel ชั้น 1', 'ตู้ Sub-Panel ชั้น 2', 'ตู้ควบคุมปั๊มน้ำบำบัด'],
      items: [
        { id: 'item1', label: 'ไฟแสดงสถานะ (Pilot Lamp R, Y, B) ติดครบทุกดวง' },
        { id: 'item2', label: 'อุณหภูมิและความร้อนสะสม (ไม่มีรอยไหม้/กลิ่นคาว)' },
        { id: 'item3', label: 'เสียงการทำงานปกติ (ไม่มีเสียง Arcing/สปาร์ค)' },
        { id: 'item4', label: 'พัดลมระบายอากาศทำงานปกติ / ตัวตู้ระบายความร้อนได้ดี' },
        { id: 'item5', label: 'สภาพทั่วไปสะอาด ประตูล็อกแน่นหนา (ไม่มีฝุ่น/สัตว์เข้าไปทำรัง)' },
      ],
    },
    gen: {
      title: 'บันทึกตรวจเช็ค: เครื่องกำเนิดไฟฟ้าสำรอง (Generator)',
      devices: ['Generator ชุดที่ 1 (อาคาร A)', 'Generator ชุดที่ 2 (สำรอง)'],
      items: [
        { id: 'item1', label: 'สวิตช์ควบคุมตั้งอยู่ที่โหมด AUTO พร้อมใช้งาน' },
        { id: 'item2', label: 'ระดับน้ำมันเชื้อเพลิง (มีมากกว่า 3/4 ถัง)' },
        { id: 'item3', label: 'ระดับน้ำยาคูลเลนท์ระบายความร้อนและน้ำมันเครื่องปกติ' },
        { id: 'item4', label: 'แรงดันแบตเตอรี่สตาร์ทปกติ (~24V - 27V)' },
        { id: 'item5', label: 'ไม่มีคราบน้ำมัน/น้ำยารั่วซึมลงพื้น และทดสอบ Test Run ปกติ' },
      ],
    },
  };

  const handleToggleCheck = (id: string, status: 'pass' | 'fail') => {
    setChecklist((prev) => ({ ...prev, [id]: status }));
  };

  const handleSave = (e: React.FormEvent) => {
    e.preventDefault();
    alert(`บันทึกการตรวจเช็ค ${selectedDevice} ประจำวันที่ ${checkDate} เรียบร้อยแล้ว!`);
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4 md:p-6 text-gray-800">
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-xl font-bold text-gray-800">Smart ENV & 5S</h1>
          <p className="text-xs text-gray-500">Water & FMS Management (HAI Standard)</p>
        </div>
        <div className="flex items-center gap-2 text-sm bg-white px-3 py-1.5 rounded-lg shadow-sm border border-gray-100">
          <span className="w-2 h-2 rounded-full bg-emerald-500"></span>
          <span>ผู้ดูแลระบบ</span>
        </div>
      </div>

      {/* แถบ Tab หลัก (เพิ่มปุ่ม บำรุงรักษา) */}
      <div className="flex flex-wrap gap-2 mb-6 bg-white p-2 rounded-xl border border-gray-100 shadow-sm">
        <button
          onClick={() => setActiveTab('quality')}
          className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${
            activeTab === 'quality' ? 'bg-sky-500 text-white shadow-sm' : 'text-gray-600 hover:bg-gray-100'
          }`}
        >
          🧪 การตรวจคุณภาพน้ำ
        </button>
        <button
          ...
          className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${
            activeTab === 'history' ? 'bg-sky-500 text-white shadow-sm' : 'text-gray-600 hover:bg-gray-100'
          }`}
        >
          📂 ประวัติการบันทึก
        </button>
        <button
          onClick={() => setActiveTab('emergencyHistory')}
          className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${
            activeTab === 'emergencyHistory' ? 'bg-sky-500 text-white shadow-sm' : 'text-gray-600 hover:bg-gray-100'
          }`}
        >
          🚨 ประวัติน้ำส่วนกลางไม่ไหล
        </button>
        <button
          onClick={() => setActiveTab('admin')}
          className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${
            activeTab === 'admin' ? 'bg-sky-500 text-white shadow-sm' : 'text-gray-600 hover:bg-gray-100'
          }`}
        >
          ⚙️ บริหารระบบ
        </button>
        
        {/* เพิ่ม Tab บำรุงรักษา */}
        <button
          onClick={() => setActiveTab('pm')}
          className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${
            activeTab === 'pm' ? 'bg-indigo-600 text-white shadow-sm font-semibold' : 'text-gray-600 hover:bg-indigo-50'
          }`}
        >
          🛠️ บำรุงรักษา (PM)
        </button>
      ...
      {/* ================= แสดงผลเนื้อหาตาม Tab ที่เลือก ================= */}
      {activeTab === 'history' && (
        <div className="space-y-4">
          <div className="flex flex-wrap gap-2">
            <button
              onClick={() => setHistorySubTab('statWastewater')}
              className={`px-3 py-1.5 rounded-lg text-xs font-semibold ${
                historySubTab === 'statWastewater' ? 'bg-orange-500 text-white' : 'bg-gray-200 text-gray-700'
              }`}
            >
          ...
          <div className="bg-white p-4 rounded-xl border border-gray-100 shadow-sm">
            <p className="text-sm text-gray-500">ตารางประวัติสถิติบำบัดน้ำเสีย (คงตารางเดิมไว้ตามปกติ)</p>
          </div>
        </div>
      )}

      {/* ================= ฟังก์ชันบำรุงรักษาอุปกรณ์ไฟฟ้า (ส่วนที่เพิ่มใหม่) ================= */}
      {activeTab === 'pm' && (
        <div className="space-y-6 max-w-4xl mx-auto">
          {/* ปุ่มสลับประเภทอุปกรณ์ */}
          <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-100">
            <label className="block text-sm font-semibold text-gray-700 mb-3">เลือกระบบที่ต้องการตรวจเช็ค</label>
            <div className="grid grid-cols-2 gap-3">
              <button
                type="button"
                onClick={() => {
                  setPmCategory('mdb');
                  setSelectedDevice(pmConfig.mdb.devices[0]);
                }}
                className={`py-3.5 px-4 rounded-xl border-2 font-bold text-sm md:text-base transition-all ${
                  pmCategory === 'mdb'
              ...
              <button
                ...
                onClick={() => {
                  setPmCategory('gen');
                  setSelectedDevice(pmConfig.gen.devices[0]);
                }}
                className={`py-3.5 px-4 rounded-xl border-2 font-bold text-sm md:text-base transition-all ${
                  pmCategory === 'gen'
                    ? 'border-indigo-600 bg-indigo-50 text-indigo-700 shadow-sm'
          ...
          <div className="bg-white rounded-xl p-5 md:p-6 shadow-sm border border-gray-100">
            <div className="flex flex-wrap justify-between items-center mb-5 gap-2 border-b pb-3">
              <h3 className="text-base md:text-lg font-bold text-gray-800">{pmConfig[pmCategory].title}</h3>
              <button
                type="button"
                className="bg-indigo-600 hover:bg-indigo-700 text-white text-xs md:text-sm font-medium px-3.5 py-2 rounded-lg flex items-center gap-1.5 transition-all shadow-sm"
              >
                📷 สแกน QR อุปกรณ์
              </button>
            </div>
            ...
            <form onSubmit={handleSave}>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-5">
                <div>
                  <label className="block text-xs font-semibold text-gray-600 mb-1">ระบุอุปกรณ์ / ตำแหน่งที่ตั้ง</label>
                  <select
                    value={selectedDevice}
                    onChange={(e) => setSelectedDevice(e.target.value)}
                    className="w-full border border-gray-300 rounded-lg p-2.5 text-sm text-gray-800 focus:ring-2 focus:ring-indigo-500 outline-none bg-white"
                  >
                    {pmConfig[pmCategory].devices.map((dev) => (
                ...
                <div>
                  <label className="block text-xs font-semibold text-gray-600 mb-1">วันที่ตรวจเช็คประจำสัปดาห์</label>
                  <input
                    type="date"
                    value={checkDate}
              ...
              {/* รายการตรวจสอบแบบ ปุ่มกดผ่าน/ไม่ผ่าน */}
              <div className="space-y-3 mb-6">
                <label className="block text-xs font-semibold text-gray-600">รายการตรวจเช็คสภาพความพร้อม</label>
                {pmConfig[pmCategory].items.map((item, index) => {
                  const status = checklist[item.id] || 'pass';
                  return (
                    <div
                      key={item.id}
                      className="flex flex-col sm:flex-row sm:items-center justify-between p-3.5 bg-gray-50 rounded-xl border border-gray-200 gap-3"
                    >
              ...
              <div className="mb-6">
                <label className="block text-xs font-semibold text-gray-600 mb-1">หมายเหตุ / สิ่งผิดปกติที่พบเพิ่มเติม</label>
                <textarea
                  rows={2}
                  value={notes}
                  onChange={(e) => setNotes(e.target.value)}
                  placeholder="ระบุเพิ่มเติม เช่น มีเสียงดังเล็กน้อย, ต้องเติมน้ำมันสำรองเพิ่ม..."
              ...
              <button
                type="submit"
                className="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-3.5 rounded-xl shadow-md text-sm md:text-base transition-all"
              >
                บันทึกการตรวจเช็ค PM ประจำสัปดาห์
              </button>
            </form>
          </div>
        </div>
