...
export default function FireCheck() {
  ...
  // สเตตัสควบคุมป๊อปอัพรายชื่อค้างตรวจและการกดซิงค์แบบประหยัด IO
...
// ระบบดึงไอดีถังที่ผ่านการตรวจในเดือนนี้ แบบคุ้มครอง IO Budget และปลดล็อกให้รีเฟรชแมนนวลได้ทันที
  const { data: monthlyCheckedIds = [], refetch: refetchMonthlyRecords, isFetching: isSyncing } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await supabase
        ...
        .select("location")
        .gte("checked_at", startOfMonthDate.toISOString());

      if (error) throw error;
      const ids = [...new Set((data || []).map((c: any) => c.location))];
      
      localStorage.setItem("fire_extinguisher_last_sync", new Date().toLocaleString("th-TH"));
      localStorage.setItem("fire_check_cached_ids", JSON.stringify(ids));
      return ids;
    },
    initialData: () => {
      try {
        const cached = localStorage.getItem("fire_check_cached_ids");
  ...
  // คำนวณหาถังดับเพลิงที่ตกหล่นยังไม่ได้ตรวจสอบ (คำนวณฝั่ง Client ประหยัดงบ)
  const uncheckedTanks = useMemo(() => {
    if (!locations) return [];
    const checkedSet = new Set(monthlyCheckedIds);
    return locations.filter((loc) => !checkedSet.has(loc.id));
  }, [locations, monthlyCheckedIds]);

  const handleManualRefresh = async () => {
    setManualTrigger(prev => prev + 1);
    await refetchMonthlyRecords();
    toast.success("อัปเดตสถานะการตรวจค้างประจำเดือนเรียบร้อย");
  };
  ...
  const createCheck = useMutation({
    ...
    onSuccess: () => {
      ...
      // บันทึกและรีเฟรชประวัติค้างตรวจหลังกดเซฟใหม่ทันทีเพื่อความแม่นยำ
      if (locationId) {
        const currentCached = JSON.parse(localStorage.getItem("fire_check_cached_ids") || "[]");
        if (!currentCached.includes(locationId)) {
          currentCached.push(locationId);
          localStorage.setItem("fire_check_cached_ids", JSON.stringify(currentCached));
          setManualTrigger(prev => prev + 1);
        }
      }
  ...
  return (
    <div className="space-y-4 pb-6">
      ...
      {/* เพิ่มส่วนการ์ดเช็กความคืบหน้าค้างตรวจประจำเดือน พร้อมปุ่มแมนนวลซิงค์ */}
      {uncheckedTanks.length > 0 ? (
        <Card 
          className="border border-amber-200 bg-amber-50 rounded-3xl shadow-sm cursor-pointer hover:bg-amber-100/70 transition-all"
          ...
          <CardContent className="p-4 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-amber-700 font-medium">ค้างตรวจอีก {uncheckedTanks.length} ถัง (ดึงข้อมูล: {localStorage.getItem("fire_extinguisher_last_sync") || "ครั้งแรก"})</p>
              </div>
            </div>
            <div className="flex items-center gap-2" onClick={(e) => e.stopPropagation()}>
              <Button 
                ...
                disabled={isSyncing}
                className="h-9 w-9 rounded-xl hover:bg-amber-200 text-amber-800"
                title="รีเฟรชอัปเดตข้อมูล"
              >
                <RefreshCw className={cn("h-4 w-4", isSyncing && "animate-spin")} />
              </Button>
              <Badge className="bg-amber-600 hover:bg-amber-700 rounded-xl px-3 py-1 text-xs">ดูรายชื่อ</Badge>
            </div>
          </CardContent>
        </Card>
      ) : (
        <Card className="border border-emerald-200 bg-emerald-50 rounded-3xl shadow-sm">
          <CardContent className="p-4 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm font-bold text-emerald-900">ยอดเยี่ยม! ตรวจครบถังดับเพลิงทุกจุดแล้ว</p>
                <p className="text-xs text-emerald-700 font-medium">ประจำเดือนนี้ไม่มีรายการค้างตรวจสอบ</p>
              </div>
            </div>
            <Button 
              variant="ghost" 
              ...
              onClick={handleManualRefresh} 
              disabled={isSyncing}
              className="h-9 w-9 rounded-xl hover:bg-emerald-200 text-emerald-800"
              title="รีเฟรชอัปเดตข้อมูล"
            >
              <RefreshCw className={cn("h-4 w-4", isSyncing && "animate-spin")} />
            </Button>
      ...
      {/* ป๊อปอัพ (Modal) แสดงรายชื่อถังดับเพลิงที่ค้างตรวจ */}
        ...
        <DialogContent className="rounded-3xl max-w-md max-h-[80vh] overflow-y-auto">
          ...
          <div className="space-y-2 mt-2">
            <p className="text-xs text-muted-foreground pb-2 border-b">
              เป้าหมาย: ต้องเข้าทำการตรวจสอบให้ครบถังภายในศุกร์สัปดาห์ที่ 2 ของเดือน
            </p>
              ...
              {uncheckedTanks.map((tank: any, index: number) => (
                <div key={tank.id} className="flex items-center justify-between p-2.5 rounded-xl bg-slate-50 border border-slate-100">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <p className="text-xs font-bold text-slate-800">{tank.name || "ถังดับเพลิง"}</p>
                      {tank.building && <p className="text-[10px] text-muted-foreground">{tank.building} - {tank.floor}</p>}
                    </div>
                  </div>
