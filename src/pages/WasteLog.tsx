...
const PIE_COLORS = ["#388c0e", "#729df1", "#ef5b8d", "#673ab7"];
...
export default function WasteLog() {
  ...
  const handleInfectiousExport = () => {
    ...
    toast.success("ส่งออกข้อมูลขยะติดเชื้อแยกรายหน่วยงานสำเร็จ");
  };
            ...
            {filterPeriod === "custom" && (
              <div className="flex flex-wrap gap-2">
                ...
                <Popover>
                  ...
                  <PopoverContent className="w-auto p-0" align="start">
                    <Calendar mode="single" selected={customTo} onSelect={setCustomTo} disabled={(d) => d > new Date() || (customFrom ? d < customFrom : false)} initialFocus className="p-3 pointer-events-auto" />
                  </PopoverContent>
                </Popover>
              </div>
            )}
                    ...
                    {combinedLogs.map((log: any) => {
                      ...
                      return (
                        <tr key={log.id} className="hover:bg-slate-50/50 transition-colors">
                          ...
                          <td className="p-4 font-bold text-slate-900">{log.weight}</td>
        ...
        <TabsContent value="infectious" className="mt-4 space-y-4">
          <div className="flex flex-wrap items-center justify-between gap-2 bg-white p-4 rounded-2xl border border-slate-100 shadow-sm">
            <div>
              ...
              <p className="text-xs text-slate-500 mt-0.5">รวมน้ำหนักตามตัวกรองช่วงเวลาด้านบน: <span className="font-bold text-orange-600">{infectiousFilteredTotal} กก.</span></p>
            </div>
            <Button size="sm" variant="outline" className="rounded-2xl text-xs h-9 gap-1" onClick={handleInfectiousExport}>
              <Download className="h-3.5 w-3.5" /> Export Excel แยกหน่วยงาน
            </Button>
          </div>
                    ...
                    {filteredInfectious.map((r: any) => {
                      ...
                      return (
                        <tr key={r.id} className="hover:bg-slate-50/50 transition-colors">
                          ...
                          <td className="p-4">{r.sharp_waste_kg || 0}</td>
                          <td className="p-4">{r.non_sharp_waste_kg || 0}</td>
                          <td className="p-4 font-bold text-orange-600">{total}</td>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <Card className="shadow-lg border-0 rounded-3xl bg-gradient-to-br from-emerald-50 to-emerald-100/60 md:col-span-1">
              <CardContent className="p-5 text-center flex flex-col justify-center h-full">
                <p className="text-3xl font-black text-emerald-700">฿{totalCost.toLocaleString()}</p>
                    ...
                    <tbody className="divide-y divide-slate-100 text-sm">
                      {Object.entries(typesMap).map(([k, v]) => {
                        ...
                        const weight = normalizedKey === "infectious"
                          ? infectiousFilteredTotal
                          : combinedLogs.filter((l: any) => normalizeWasteType(l.waste_type) === normalizedKey).reduce((s: number, l: any) => s + Number(l.weight), 0);
                        ...
                        return (
                          <tr key={k} className="hover:bg-slate-50/50 transition-colors">
                            ...
                            <td className="p-4 font-semibold text-slate-700">{Math.round(weight * 100) / 100}</td>
                            <td className="p-4 text-slate-500">฿{rate}</td>
                            <td className="p-4 text-right font-bold text-slate-900">฿{Math.round(amt * 100) / 100}</td>
                          </tr>
                        );
                      })}
                    </tbody>
              ...
              <div className="space-y-4 border border-orange-100 bg-orange-50/30 p-4 rounded-2xl">
                <div className="flex flex-wrap items-center justify-between gap-2 border-b border-orange-100 pb-3">
                  <div>
                    ...
                    <p className="text-xs text-orange-700/80 mt-0.5">กรอกข้อมูลแยกแต่ละ รพ.สต. ระบบจะรวมน้ำหนักลงทะเบียนรวมให้โดยอัตโนมัติ</p>
                  </div>
              ...
              <div className="p-4 bg-amber-50/50 border border-amber-200/60 rounded-2xl space-y-3">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  <div className="space-y-1">
                    <Label className="text-[11px] font-semibold text-slate-600">กำหนดวันเวลาลงบันทึกเจาะจง (ปล่อยว่างไว้หากต้องการใช้วันนี้)</Label>
                    <Input
                      type="datetime-local"
