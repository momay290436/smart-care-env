...
import { XAxis, YAxis, Tooltip, ResponsiveContainer, PieChart, Pie, Cell, Legend, CartesianGrid, BarChart, Bar } from "recharts";
...
export default function WasteLog() {
  ...
  // เรียงลำดับจากวันที่รับขยะล่าสุดไปเก่าสุด
  ...
  return (
    <div className="container max-w-5xl mx-auto px-4 space-y-4 pb-10">
      <PageHeader title="ระบบจัดการขยะ" subtitle="บันทึกข้อมูล ปริมาณขยะ และขยะติดเชื้อ">
        ...
        <Button size="sm" className="h-9 rounded-2xl text-xs gap-1" onClick={() => { setEditingLog(null); setShowForm(!showForm); }}><Plus className="h-3.5 w-3.5" /> {showForm ? "ซ่อนฟอร์ม" : "บันทึกขยะ"}</Button>
      </PageHeader>
